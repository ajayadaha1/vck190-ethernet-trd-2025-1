...
entity axi4_full_v1_0_M00_AXI is
	generic (
	...
	-- Input Base address for timestamp write	
		BASE_ADDR: in std_logic_vector (63 downto 0);
	-- Input timestamp data
		START_DATA: in std_logic_vector (95 downto 0);
	-- Output port to start memory write transaction
		INIT_MEM_TXN : out std_logic;
	-- Input empty signal from FIFO
		FIFO_EMPTY : in std_logic
	);
end axi4_full_v1_0_M00_AXI;

architecture implementation of axi4_full_v1_0_M00_AXI is
	...
	-- total number of burst transfers is master length divided by burst length and burst size
	 constant  C_NO_BURSTS_REQ  : integer := 1 ; --(C_MASTER_LENGTH-clogb2((C_M_AXI_BURST_LEN*C_M_AXI_DATA_WIDTH/8)-1));
	...
	signal write_index	: std_logic_vector(C_TRANSACTIONS_NUM downto 0):= (others => '0');
	...
	constant lsb_bit_num: integer := 0;
	constant msb_bit_num: integer := 31;
...
begin
	...
	M_AXI_AWADDR	<= std_logic_vector( unsigned(BASE_ADDR) + unsigned(axi_awaddr) );
	...
	--Read Address (AR)
	M_AXI_ARID	<= (others => '0');
	M_AXI_ARADDR	<= std_logic_vector( unsigned( BASE_ADDR ) + unsigned( axi_araddr ) );
	--Burst LENgth is number of transaction beats, minus 1
...
process(M_AXI_ACLK)                                                          
	begin                                                                             
	  if (rising_edge (M_AXI_ACLK)) then                                              
	    ...
	    if (M_AXI_ARESETN = '0' ) then                                                
            axi_txn <= '0';                                                          
	    elsif (INIT_AXI_TXN = '1' and compare_done = '0' and axi_txn = '0') then
	        axi_txn <= '1';
	    elsif (compare_done = '1') then
	      axi_txn <= '0';                                                                                                                                             
	    end if;                                                                       
	  end if;                                                                         
end process; 
	...
	-- Data pattern is only a simple incrementing count from 0 for each burst  */       
	  ...
	  begin                                                                             
	    if (rising_edge (M_AXI_ACLK)) then                                              
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                   
            axi_wdata <= START_DATA (31 downto 0);                                                         
	      elsif (wnext = '1' and write_index = "00") then                                                       
	          axi_wdata <= START_DATA (63 downto 32);
	      elsif (wnext = '1' and write_index = "01") then  
	           axi_wdata <= START_DATA (95 downto 64); 
	      else
	           axi_wdata <= axi_wdata;                                    
	      end if;                                                                     
	    end if;                                                                                                                                              
	  ...
	  begin                                                  
	    if (rising_edge (M_AXI_ACLK)) then                   
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                     
	        ...
	        expected_rdata <= START_DATA (31 downto 0);  
            elsif (rnext = '1' and read_index = "00") then
              expected_rdata <= START_DATA (63 downto 32);
            elsif (rnext = '1' and read_index = "01") then
              expected_rdata <= START_DATA (95 downto 64); 
            else
	      ...
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                                         
	       writes_done <= '0';                                                                                                                          
	        ...
	        if (M_AXI_BVALID = '1' and axi_bready = '1') then   
	          writes_done <= '1';
	          --- making writes_done zero when no response from slave
	        else
	          writes_done <= '0';                                                                                 
	        end if;                                                                                              
	      end if;                                                                                                
